...
if (program.flags.includes('--compile')) {
  ...
  $ bytenode -c script.js             compile \`script.js\` to \`script.bin\`.
  $ bytenode -c server.js app.js
  $ bytenode -c src/*.js              compile all \`.js\` files in \`src/\` directory.

  $ bytenode script.bin [arguments]   run \`script.bin\` with arguments.
  $ bytenode                          open Node REPL with bytenode pre-loaded.

  $ echo 'console.log("Hello");' | bytenode --compile - > hello.bin
                                      compile from stdin and save to \`hello.bin\`
`);
